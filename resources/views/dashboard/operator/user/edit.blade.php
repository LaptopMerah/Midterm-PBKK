    ...
    <div class="p-4 bg-white border border-gray-200 rounded-lg shadow-sm dark:border-gray-700 sm:p-6 dark:bg-gray-800">
        ...
        <div class="items-center justify-between lg:flex flex-col lg:flex-row">
            <div class="w-full lg:w-1/4 mb-4 lg:mb-0">
                <h3 class="mb-2 text-xl font-bold text-gray-900 dark:text-white">Edit user data</h3>
                <span class="text-base font-normal text-gray-500 dark:text-gray-400">
                    Change data of user, {{$user->name}}
                </span>
            </div>
        </div>
        <div class="">
            <section>
                <form method="post" action="{{ route('operator.user-management.update',$user->id) }}" class="mt-6 space-y-6">
                    @method('put')
                    @csrf
                    <div>
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="identifier_number" class="block mt-1 w-full" type="tel" pattern="[0-9]{10}"
                                      ...
                                      :value="old('identifier_number',$user->identifier_number)" required/>
                    ...
                    <div class="mt-4">
                        ...
                        <x-text-input id="phone_number" class="block mt-1 w-full" type="tel" pattern="[0-9]{10-15}"
                                      maxlength="15" minlength="10" name="phone_number" :value="old('phone_number',$user->phone_number)"
                                      required/>
                    ...
                    <div>
                        ...
                        <x-select-input id="user_type" name="user_type">
                            @foreach($userType as $user_type)
                                @if(old('$user_type',$user->user_type) == $user_type)
                                    <option value="{{$user_type}}"
                                            selected>{{\Illuminate\Support\Str::ucfirst($user_type->value)}}</option>
                                @endif
                                <option
                                    value="{{$user_type}}">{{\Illuminate\Support\Str::ucfirst($user_type->value)}}</option>
                            @endforeach
                        </x-select-input>
